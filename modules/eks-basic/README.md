...
Ce module déploie le cluster Amazon EKS, ses Node Groups privés et configure les politiques d accès IAM (Access Entries) et le provider OIDC.
...
|------|-------------|------|---------|:--------:|
| <a name="input_admin_roles"></a> [admin\_roles](#input\_admin\_roles) | Map des noms et ARNs de rôles IAM à qui donner les droits cluster-admin via EKS Access Entries | `map(string)` | `{}` | no |
...
| <a name="input_irsa_roles"></a> [irsa\_roles](#input\_irsa\_roles) | Map des rôles IRSA à créer pour les workloads EKS | <pre>map(object({<br/>    role_name          = string<br/>    service_account    = string<br/>    namespace          = string<br/>    policy_json        = string<br/>  }))</pre> | `{}` | no |
...
| <a name="output_irsa_role_arns"></a> [irsa\_role\_arns](#output\_irsa\_role\_arns) | Map des ARNs des rôles IRSA créés |
